class Level3 extends Phaser.Scene {
    ...
    create() {
        ...
        this.map = this.add.tilemap("platformer-level-3", 18, 18, 100, 20);
       ...
       // Add a tileset to the map
        ...
        this.tileset1 = this.map.addTilesetImage("kenny_tilemap_packed_extended", "tilemap_tiles_extended");
        ...
        // object for flag, marks the end goal and ends the game when touched
        this.cannon = this.map.createFromObjects("Objects", {
            name: "cannon",
            key: "tilemap_sheet_extended",
            frame: 64
        });
        ...
        this.physics.world.enable(this.cannon, Phaser.Physics.Arcade.STATIC_BODY);
    
        // Create a Phaser group out of the array this.coins
        // This will be used for collision detection below.
        this.coinGroup = this.add.group(this.coins);
        this.flagGroup = this.add.group(this.flag);
        this.cannonGroup = this.add.group(this.cannon);
        this.bullets = this.add.group();

        // placing every tile that's a cannon in a group variable to iterate through
        this.cannons = this.cannonGroup.getChildren();
        ...
        this.sKey = this.input.keyboard.addKey('S');  //method to advance to next level
        ...
        document.getElementById('description').innerHTML = '<h2>Level 3! Watch out for the snow cannons!!</h2><br>Use the arrow keys to jump and move!</br>';
    ...
    update() {
        ...
        if(this.winState == true) {
            my.sprite.player.setAccelerationX(0);
            document.getElementById('description').innerHTML = '<h1>Level Complete!!! Congradulations, you beat the game!</h1><br>Press S to restart the game!</br>';
        ...
        if (this.deathState ==false) {
            if (this.fireCannon == true) {
            
                // for loop to iterate over each cannon to activate the shootBullet function
                for (let i = 0; i < this.cannons.length; i++) {
                    let gun = this.cannons[i];
                    this.shootBullet(gun);  
                }
    ...
    //functon to let each cannon shoot a bullet
    shootBullet(cannon){
        let bullet = this.physics.add.sprite(cannon.x, cannon.y, 'bullet');
        bullet.setVelocityX(-200);
        bullet.body.allowGravity = false;
        this.bullets.add(bullet);
        this.physics.add.collider(bullet, this.groundLayer, (obj1, obj2) => {
            obj1.destroy();
            });
        this.physics.add.collider(bullet, my.sprite.player, (obj1, obj2) => {
            this.deathState = true;
            obj1.destroy();
            });
    }
